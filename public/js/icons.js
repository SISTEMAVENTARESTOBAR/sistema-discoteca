// Sistema centralizado de iconos SVG - profesional y consistente
const ICONS = {
  ...
  cash: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>',
  ...
  table: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="9" y1="3" x2="9" y2="21"/></svg>',
  ...
  chefHat: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 13.87A4 4 0 0 1 7.41 6a5.11 5.11 0 0 1 1.05-1.54 5 5 0 0 1 7.08 0A5.11 5.11 0 0 1 16.59 6 4 4 0 0 1 18 13.87V20H6z"/><line x1="6" y1="17" x2="18" y2="17"/></svg>',
  wine: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 22h8"/><path d="M12 11v11"/><path d="M17 2H7l5 9.75z"/></svg>',
  beer: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 11h1a3 3 0 0 1 0 6h-1"/><path d="M9 12v6"/><path d="M13 12v6"/><path d="M14 7.5c-1 0-1.44.5-3 .5s-2-.5-3-.5-1.72.5-2.5.5a2.5 2.5 0 1 1 0-5c.78 0 1.57.5 2.5.5S9.44 2 11 2s2 1.5 3 1.5 1.72-.5 2.5-.5a2.5 2.5 0 0 1 0 5c-.78 0-1.5-.5-2.5-.5z"/><path d="M5 8v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V8"/></svg>',
...
// Función helper para obtener icono con tamaño y clase
function icon(name, size = 16, className = '') {
  let svg = ICONS[name] || '';
  // Add explicit width/height to SVG for proper sizing
  if (svg) {
    svg = svg.replace('<svg ', `<svg width="${size}" height="${size}" `);
  }
  return `<span class="icon ${className}" style="width:${size}px;height:${size}px;">${svg}</span>`;
}

// Atajos para los más usados
const iconCheck = (s=14) => icon('check', s, 'icon-success');
const iconX = (s=14) => icon('x', s, 'icon-danger');
const iconAlert = (s=14) => icon('alertTriangle', s, 'icon-warning');
const iconInfo = (s=14) => icon('info', s, 'icon-info');
const iconClock = (s=14) => icon('clock', s, 'icon-muted');
const iconCash = (s=14) => icon('cash', s, 'icon-success');
const iconPhone = (s=14) => icon('smartphone', s, 'icon-info');
const iconUser = (s=14) => icon('user', s, 'icon-muted');
const iconNote = (s=14) => icon('note', s, 'icon-muted');
const iconTrash = (s=14) => icon('trash', s, 'icon-danger');
const iconEdit = (s=14) => icon('edit', s, 'icon-info');
const iconChevronDown = (s=14) => icon('chevronDown', s, 'icon-muted');
const iconChevronUp = (s=14) => icon('chevronUp', s, 'icon-muted');
