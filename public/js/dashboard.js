...
function renderDashboard() {
  ...
  if (alertas.length === 0) {
    ac.innerHTML = '<div class="alert alert-success"><span class="alert-icon">✅</span><div class="alert-text"><strong>Sin alertas activas</strong><span>Todo en orden esta noche</span></div></div>';
  } else {
  ...
  if (sorted.length === 0) {
    tp.innerHTML = '<div class="empty-state"><div class="empty-icon">📦</div><p>Sin ventas hoy</p></div>';
  } else {
...
function getAlertas() {
  ...
  DB.usuarios.filter(u => u.rol === 'garzon').forEach(g => {
    ...
    if (count >= 3) {
      alertas.push({ tipo: 'danger', icon: '🔴', titulo: `${g.nombre} — ${count} anulaciones esta noche`, detalle: 'Límite alcanzado — no puede anular más pedidos' });
    }
  });
  DB.mesas.filter(m => m.estado !== 'libre' && m.estado !== 'entregado').forEach(m => {
    ...
    if (pedido) {
      ...
      if (elapsed > 60) {
        ...
        alertas.push({ tipo: 'warning', icon: '🟡', titulo: `Mesa ${m.numero} — ${g ? g.nombre : ''}`, detalle: `Pedido abierto hace ${elapsed} min sin cobrar` });
      }
    }
  });
  return alertas;
